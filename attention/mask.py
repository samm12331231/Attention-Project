...
def visualize_attentions(tokens, attentions):
    for layer_number, layer_attn in enumerate(attentions, start=1):
        num_heads = layer_attn.shape[1]
        for head_number in range(num_heads):
            attention_weights = layer_attn[0][head_number].numpy()
            generate_diagram(layer_number, head_number + 1, tokens, attention_weights)
